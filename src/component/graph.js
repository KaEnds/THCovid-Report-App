...
export default function Graph({ data }) {
  ...
  const label = spreadData(
    dataSet.map((txn) => txn.txn_date),
    parseInt(day),
    findfirstindex,
    findlastindex
  );
  const Case = spreadData(
    dataSet.map((Case) => Case.new_case),
    parseInt(day),
    findfirstindex,
    findlastindex
  );
  const Death = spreadData(
    dataSet.map((Case) => Case.new_death),
    parseInt(day),
    findfirstindex,
    findlastindex
  );
  const Recovered = spreadData(
    dataSet.map((Case) => Case.new_recovered),
    parseInt(day),
    findfirstindex,
    findlastindex
  );
